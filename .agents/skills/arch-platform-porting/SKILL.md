...
- AArch64 异常级 1 启动与运行时内存管理单元初始化必须显式清除 `SCTLR_EL1.A`，不能继承固件的对齐检查策略；Linux 兼容用户空间依赖 Normal 内存上的普通非对齐访问。设置用户缓存指令访问位和 SPAN 时，在最终寄存器值中验证 A 位确实为零，不能依赖零值字段参与 `modify` 组合。
