...
echo "$profile_start_status" | grep -q 'enabled=true phase=1' \
...
profile_records="$(grep -Ec '^STARRY_(CPU|WAIT) ' "$artifact_dir/kernel-profile.raw" || true)"
