...
4. StarryOS owns the sleepable lease task because `ax-driver` cannot depend on
   ...
   CPU affinity and automatic build parallelism, matching the Linux baseline.
...
every 10 seconds from CPU 0. The guest command timeout is 1,800 seconds and the
watchdog lease is 1,860 seconds, leaving one minute for profile capture and
shutdown. The lease is the outer bound when shutdown or the
