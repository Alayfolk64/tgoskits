...
#[cfg(feature = "selfbuild-watchdog")]
const SELFBUILD_WATCHDOG_LEASE: core::time::Duration = core::time::Duration::from_secs(1_860);
...
fn spawn_selfbuild_watchdog() {
    ...
    {
        let mut affinity =
            ax_runtime::task::sched::CpuSet::empty(ax_runtime::hal::cpu_num());
        assert!(
            affinity.insert(ax_runtime::task::sched::CpuId::new(0)),
            "self-build watchdog cannot select CPU 0"
        );
        kernel_thread_builder(String::from("selfbuild-wdt"))
            .affinity(affinity)
            .spawn(selfbuild_watchdog_loop)
            .unwrap_or_else(|error| panic!("failed to spawn self-build watchdog: {error}"));
    }
}
